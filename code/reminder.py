...
def run(message, bot):
    ...
    if history is None or history == []:
        ...
        msg = bot.reply_to(
            message,
            "Please select a category to set reminders for",
            reply_markup=markup,
        )
...
def process_reminder_type(message, chat_id, bot):
    ...
    bot.send_message(
        chat_id,
        "Please enter the time for the reminder in the format HH:MM (e.g., 14:30)",
    )
    bot.register_next_step_handler(
        message, process_reminder_time, chat_id, selected_type, bot
    )
...
def process_reminder_time(message, chat_id, selected_type, bot):
    ...
    if helper.validate_time_format(reminder_time_str):
        ...
        bot.send_message(
            chat_id,
            f"Your {selected_type} expenses reminder has been set for {reminder_time_str}.",
        )
    else:
        bot.send_message(
            chat_id,
            "Invalid time format. Please try /set_reminder with the time in the format HH:MM (e.g., 14:30)",
        )
...
def check_reminders(bot):
    ...
    for chat_id, data in json_data.items():
        ...
        if reminder and reminder.get("time"):
            ...
            if (
                current_time.hour == reminder_time.hour and current_time.minute == reminder_time.minute
            ):
                reminder_type = reminder.get("type")
                if (
                    chat_id,
                    current_date,
                    str(reminder_time.hour) + ":" + str(reminder_time.minute),
